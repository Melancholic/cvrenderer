...
- **`internal/typst`** — the only code that runs the `typst` binary, and `Render(templatePath, dataPath)` is its whole surface: it compiles a template against a data file and streams the PDF from typst's stdout. **Neither the template nor the data file is fixed on the `Renderer` struct — both are passed per call**, because the HTTP layer selects them per request. There is no render-from-bytes path: the service only renders files that already exist under `DATA_DIR`, so nothing hits disk on either side.
...
- **`photo` must be a root-absolute path.** Typst resolves `image()` paths *relative to the template file* (`templates/`), not the data file — so a bare `data/photo.svg` would look for `templates/data/photo.svg`. The `photo:` field must therefore be root-absolute (`/data/photo.svg` → `$ROOT/data/photo.svg`), matching how the data file itself is addressed. `data/photo.svg` is a shipped placeholder (a generic silhouette, not a real face); both templates read the same field — `helsinki` clips it into its circular avatar, `primeats` shows it as a top-right rectangle.
